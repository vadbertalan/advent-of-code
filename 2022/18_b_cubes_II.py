...
def can_reach_wall_of_cage(coord: Coord3, visited):
    global exposed

    visited.add(coord.get_tuple())

    if coord.get_tuple() in exposed:
        return True
    ...
    ) not in cubes_coord_map and coord.get_tuple() not in visited, surrounding_coords))
    ...
    if any(map(lambda coord: coord.get_tuple() in exposed, surrounding_coords)):
    ...
    for surrounding_coord in eligible_surrounding_coords:
        if surrounding_coord.get_tuple() not in visited:
            if can_reach_wall_of_cage(surrounding_coord, visited):
                exposed.add(surrounding_coord.get_tuple())
                return True
...
exposed_cubes = []
for cube in cubes:
    if can_reach_wall_of_cage(cube, set()):
        exposed_cubes.append(cube)
print('exposed count', len(exposed_cubes))

solution = 0
for coord in exposed_cubes:
    surrounding_coords = [
        Coord3(coord.x, coord.y, coord.z + 1),  # up
        Coord3(coord.x + 1, coord.y, coord.z),  # right
        Coord3(coord.x, coord.y, coord.z - 1),  # down
    ...
    eligible_surrounding_coords = list(
        filter(lambda coord: coord.get_tuple() not in cubes_coord_map, surrounding_coords))

    for c in eligible_surrounding_coords:
        if can_reach_wall_of_cage(c, set()):
            solution += 1
