...
class Node:
    ...
    def __str__(self):
        return f'[{self.left.__str__()},{self.right.__str__()}]' if self.is_pair else str(self.data)
...
def reduce(node, depth=0):
    ...
    if depth == 4:
        # print('reducing node ', node)
        l, r = node.left.data, node.right.data
        node.is_pair = False
        node.left = node.right = None
        node.data = 0
        return l, r, True
    if node.left and node.left.is_pair:
        l, r, ch = reduce(node.left, depth + 1)

        change_happened = ch

        if l or r:
            if r:
                node.right.leftmost().data += r
            return (l, None, ch)

        if change_happened:
            return (None, None, True)
    if node.right and node.right.is_pair:
        l, r, ch = reduce(node.right, depth + 1)

        change_happened = ch

        if l or r:
            if l:
                node.left.rightmost().data += l
            return (None, r, ch)

        if change_happened:
            return (None, None, True)
    return (None, None, change_happened)
...
def split(node):
    ...
    if node.right:
        ...
        if ch:
            return ch
    if node.data and node.data >= 10:
        # print('splitting node ', node)
        l = math.floor(node.data / 2)
        r = math.ceil(node.data / 2)
        node.is_pair = True
        node.left = Node(l)
        node.right = Node(r)
        node.data = None
...
max = -1
d = [[0 for _ in range(len(input))] for _ in range(len(input))]
for i in range(len(input)):
    for j in range(len(input)):
        if i != j:
            s = get_magnitude(add([input[i], input[j]]))
            if s > max:
                max = s
